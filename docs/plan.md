...
### 私（Claude Code）が完了したタスク
- [x] README整備（環境変数・セットアップ手順・CLIデモ・本番URL記載） (done: 2026-05-06)
- [x] 社内向け操作マニュアル `docs/manual.md` 作成（Notionにコピペ可能なMarkdown） (done: 2026-05-06)
- [x] 本番Vercelデプロイ Smokeテスト（HTTP 200 / フォーム表示確認） (done: 2026-05-06)
- [x] 本番URL確定: https://meo-text-tool.vercel.app (done: 2026-05-06)

### ユーザー側で実施するタスク
- [ ] **社内3名以上で実案件テスト**（実工務店URLで品質検証）
- [ ] フィードバック収集（生成テキスト品質、操作性、要望）
- [ ] `docs/manual.md` をNotionに公開（社内ナレッジベース）
- [ ] gbp-eight.vercel.app の置き換えアナウンス（チャット/メールで通知）
- [ ] 必要に応じてプロンプト微調整（フィードバック起点）

**完了条件**: 社内利用開始可能 ✓（コード/ドキュメント側は完了、社内テストはユーザー側で実施）
...
## Phase 6: 運用フィードバック反映（随時）

### 2026-05-07 KW・商品ロジック改善
- [x] KW提案数 10個 → 20個 に拡張 (done: 2026-05-07)
- [x] area カテゴリ最低3個保証 (done: 2026-05-07)
- [x] ロングテール / クエリファンアウト対応プロンプト改訂 (done: 2026-05-07)
- [x] 商品抽出にモデルハウス/個別相談/保証/見学等のお問い合わせ動線ページを追加 (done: 2026-05-07)
- [x] ブログ/コラム/ニュース系ページを全工程で除外 (done: 2026-05-07)
